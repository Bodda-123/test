...
import { VisualSim } from "./visual_sim";
...
let visualSim: VisualSim;

function setupVisualSim() {
	const container = document.getElementById("simContainer");
	if (container) {
		visualSim = new VisualSim(container);
	}

	const btnStartScan = document.getElementById("btnStartScan");
	if (btnStartScan) {
		btnStartScan.onclick = () => {
			if (visualSim) visualSim.startScan();
		}
	}
	...
	// Hook into existing buttons to reset variants
	if (btnLayout) {
